...
from resultnumberdoing import numberfilter
...
def draw_result(mode,orgimg,dict_list,is_color=False):   # 车牌结果画出来
    result_str =""
    ...
    insert_to_db_threading=threading.Thread(target=numberfilter,args=(result_str,mode))
    insert_to_db_threading.start()
...
def update_image_label(img, label):
    ...
    img_tk = ImageTk.PhotoImage(image=img_pil)  # Convert to ImageTk
    label.config(image=img_tk)
    label.image = img_tk  # Keep a reference to avoid garbage collection

def plate_detection(file_path,label,mode):
    ...
    parser.add_argument('--image_path', type=str, default="", help='source')     #图片路径
    ...
    parser.add_argument('--video', type=str, default=file_path, help='source')                       #视频的路径
    ...
    if not opt.video:     #处理图片
        ...
        if capture.isOpened():
            ...
            while True:
                ...
                ori_img = draw_result(mode, img, dict_list, is_color=opt.is_color)
